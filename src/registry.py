        ...
        <model_name>/
            <version>/          e.g. v20260310_143022/
                *.joblib
                ...
                checksums.sha256
                checksums.json
            latest.txt          contains the name of the latest version dir
...
class ModelRegistry:
    """File-system model registry with versioned artifact storage."""
    ...
    def register(
        ...
        """Copy all files from *source_dir* into a new versioned slot.

        Args:
            source_dir: Directory containing trained artifacts.
            version:    Override version string (default: UTC timestamp).
    ...
    def latest_path(self) -> Path | None:
        """Return the directory of the latest version, or None."""
    ...
    def get_version_path(self, version: str) -> Path:
        """Return the path for *version*, raising FileNotFoundError if absent."""
        p = self.root / version
        if not p.exists():
    ...
    def load_metrics(self, version: str | None = None) -> dict:
        """Load metrics.json for *version* (defaults to latest)."""
