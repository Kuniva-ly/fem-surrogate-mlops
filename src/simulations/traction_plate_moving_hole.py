...
def write_dataset_chunk(df: pd.DataFrame, out_dir: Path, part_idx: int, output_format: str = "parquet") -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "Parquet export requires a parquet engine. Install 'pyarrow' (recommended) or 'fastparquet'."
        ) from exc
...
def _validate_hole_inside_plate(
    length_m: float,
    height_m: float,
    hole_radius_ratio: float,
    hole_cx_ratio: float,
    hole_cy_ratio: float,
) -> tuple[float, float, float]:
    radius = hole_radius_ratio * min(length_m, height_m)
    if radius <= 0.0:
        raise ValueError("hole_radius_ratio must be > 0")
    if radius >= 0.5 * min(length_m, height_m):
        raise ValueError("hole_radius_ratio too large: hole must stay inside the plate")

    cx = hole_cx_ratio * length_m
    cy = hole_cy_ratio * height_m

    if not (radius < cx < length_m - radius):
        raise ValueError("hole_cx_ratio places the hole outside plate bounds")
    if not (radius < cy < height_m - radius):
        raise ValueError("hole_cy_ratio places the hole outside plate bounds")
...
def solve_plate_fenics(
    ...
    hole_cx_ratio: float = 0.5,
    hole_cy_ratio: float = 0.5,
) -> tuple[float, float]:
    fenics = _try_import_fenics()
    if fenics is None:
    ...
    hole_radius_m, cx, cy = _validate_hole_inside_plate(
        length_m, height_m, hole_radius_ratio, hole_cx_ratio, hole_cy_ratio
    )
    ...
    try:
        if MPI.COMM_WORLD.rank == 0:
            ...
            gmsh.model.add("plate_with_moving_hole")
    ...
    facet_tag = mesh.meshtags(domain, fdim, right_facets, np.full(len(right_facets), 1, dtype=np.int32))
    ...
    problem = LinearProblem(
        ...
        Lform,
        bcs=[bc],
        petsc_options_prefix="elasticity_moving_hole_",
...
def solve_plate_proxy(
    ...
    radius, cx, cy = _validate_hole_inside_plate(length_m, height_m, hole_radius_ratio, hole_cx_ratio, hole_cy_ratio)
    ...
    dist_left = cx - radius
    dist_right = length_m - (cx + radius)
    dist_bottom = cy - radius
    dist_top = height_m - (cy + radius)
    min_edge_dist = max(min(dist_left, dist_right, dist_bottom, dist_top), 1e-12)
    edge_effect = 1.0 + 0.35 * min(radius / min_edge_dist, 2.0)

    mesh_factor = 1.0 + 20.0 / max(mesh_nx + mesh_ny, 1)
    vm = traction_pa * poisson_factor * mesh_factor * stress_concentration * edge_effect
    return float(displacement), float(vm)


def _sample_center_ratio(rng: np.random.Generator, lo_user: float, hi_user: float, margin: float) -> float:
    lo = max(lo_user, margin)
    hi = min(hi_user, 1.0 - margin)
    if lo >= hi:
        return 0.5
...
def sample_parameters(rng: np.random.Generator, sampling_mode: str, custom_ranges: dict) -> dict:
    ...
    cx_lo_user, cx_hi_user = custom_ranges["hole_cx_ratio"]
    cy_lo_user, cy_hi_user = custom_ranges["hole_cy_ratio"]
    ...
    if sampling_mode == "continuous":
        ...
        margin_x = hole_radius_ratio * min(length_m, height_m) / length_m
        margin_y = hole_radius_ratio * min(length_m, height_m) / height_m
        hole_cx_ratio = _sample_center_ratio(rng, cx_lo_user, cx_hi_user, margin_x)
        hole_cy_ratio = _sample_center_ratio(rng, cy_lo_user, cy_hi_user, margin_y)
        ...
        return {
            ...
            "hole_cx_ratio": hole_cx_ratio,
            "hole_cy_ratio": hole_cy_ratio,
        }
...
def generate_batch(
    ...
    seed: int,
    out_root: Path,
    data_version: str = "sim_v2_moving_hole",
    backend: str = "auto",
    ...
    while generated < n:
        ...
        for _ in range(current_chunk):
            ...
            solver_params = {
                ...
                "hole_cx_ratio": params["hole_cx_ratio"],
                "hole_cy_ratio": params["hole_cy_ratio"],
            }
            if backend == "fenics":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plate traction FEM simulation with moving hole (single or batch).")
    ...
    parser.add_argument("--data-version", default="sim_v2_moving_hole")
    ...
    parser.add_argument("--hole-cx-ratio", type=float, default=0.5, help="Fixed hole center x ratio in single mode")
    parser.add_argument("--hole-cy-ratio", type=float, default=0.5, help="Fixed hole center y ratio in single mode")
    ...
    parser.add_argument(
        "--hole-cx-range",
        default="0.1,0.9",
        help="Hole center x ratio range in batch mode. Final bounds are clipped to keep the hole inside.",
    )
    parser.add_argument(
        "--hole-cy-range",
        default="0.1,0.9",
        help="Hole center y ratio range in batch mode. Final bounds are clipped to keep the hole inside.",
    )
    ...
    if not (0.0 < args.hole_cx_ratio < 1.0):
        raise ValueError("--hole-cx-ratio must be in (0, 1)")
    if not (0.0 < args.hole_cy_ratio < 1.0):
        raise ValueError("--hole-cy-ratio must be in (0, 1)")
    ...
    hole_cx_range = _parse_range(args.hole_cx_range, float)
    hole_cy_range = _parse_range(args.hole_cy_range, float)
    ...
    if not (0.0 < hole_cx_range[0] < 1.0 and 0.0 < hole_cx_range[1] < 1.0):
        raise ValueError("--hole-cx-range bounds must both be in (0, 1)")
    if not (0.0 < hole_cy_range[0] < 1.0 and 0.0 < hole_cy_range[1] < 1.0):
        raise ValueError("--hole-cy-range bounds must both be in (0, 1)")
    ...
    custom_ranges = {
        ...
        "hole_cx_ratio": hole_cx_range,
        "hole_cy_ratio": hole_cy_range,
    }

    if args.out is None:
        run_single_default_case(
            args.backend,
            args.hole_radius_ratio,
            args.hole_cx_ratio,
            args.hole_cy_ratio,
            args.mesh_nx,
            args.mesh_ny,
