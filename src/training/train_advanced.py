"""Advanced surrogate training: LightGBM + log-transform targets + Optuna tuning.

Key improvements over train_baseline.py
----------------------------------------
1. Log-transform targets before training.
   - max_von_mises_pa spans ~4 decades (1e4 → 4e8 Pa).
   - max_displacement_m spans ~6 decades (1e-8 → 1e-2 m).
   - RMSE in log-space weights all regimes equally.
   - Predictions are back-transformed (10**pred) for final evaluation.

2. LightGBM instead of RandomForest.
   - 5-10x faster training on 50k rows.
   - Typically 5-15 R² points better on tabular physics data at this scale.

3. Optuna hyperparameter optimisation.
   - 60 trials by default, 5-fold CV, minimises RMSE on log(target).
   - Searches: n_estimators, learning_rate, num_leaves, max_depth,
...
4. Separate models per target (displacement vs von Mises) — each target has
   very different physics drivers so independent models work better than
   MultiOutputRegressor here.

5. Physics sanity checks on predictions (PDP monotonicity).

Usage
-----
python -m src.training.train_advanced \
    --data-dir data/processed \
    --out-dir  data/models/advanced \
    --n-trials 60
...
python -m src.training.train_advanced \
    ...
    --out-dir  data/models/advanced \
    --n-trials 60 \
    --mlflow --mlflow-run-name lgbm-advanced
"""
import argparse
import os
import random
...
# Physics monotonicity constraints for LightGBM.
# +1 = target increases with this feature
# -1 = target decreases with this feature
#  0 = no constraint
# Applied to both models; features absent in X are silently ignored.
_MONO_CONSTRAINTS: dict[str, int] = {
    "traction_pa":       +1,   # more load  -> more stress & more displacement
    ...
    "delta_theory":      +1,
    "epsilon":           +1,
    "hole_radius_ratio": +1,   # bigger hole -> more stress concentration
    "d_over_W":          +1,
    "Kt_theory":         +1,
...
def _encode_categoricals(
    ...
    """Ordinal-encode categorical columns. Returns (train, val, test, encoder).

    The encoder is fit ONLY on the training split to prevent data leakage.
...
def _train_one_target(
    ...
    """Tune + fit one LightGBM model for a single (log-transformed) target."""
    print(f"\n{'='*60}")
    ...
    if n_trials > 0:
        ...
        # Sensible defaults when Optuna is disabled
        best_params = {
...
def train_advanced(
    ...
    # ── Seed all RNG sources for full determinism ─────────────────────────
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train advanced LightGBM surrogate with log-transform and Optuna tuning."
    )
    parser.add_argument("--data-dir",  type=Path, default=Path("data/processed"))
    parser.add_argument("--out-dir",   type=Path, default=Path("data/models/advanced"))
    parser.add_argument("--n-trials",  type=int,  default=60,
                        help="Number of Optuna trials per target (0 = use defaults).")
